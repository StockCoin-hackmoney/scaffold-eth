...
abstract contract CustomIntegration is BaseIntegration, ReentrancyGuard, ICustomIntegration {
  ...
  function enter(
    ...
    // Post actions
    (targetAddressP, callValueP, methodDataP) = _getPostActionCallData(_strategy, customInfo.addressParam, customInfo.resultTokensInTransaction, 0);
    ...
    emit CustomEntered(address(customInfo.strategy), address(customInfo.garden), address(this), customInfo.resultToken);
  }
  ...
  function exit(
    ...
    // Pre actions
    (address targetAddressP, uint256 callValueP, bytes memory methodDataP) = _getPreActionCallData(_strategy, customInfo.addressParam, _resultTokensIn, 1);
    if (targetAddressP != address(0)) {
      // Approve spending of the pre action token
    ...
    (address target, uint256 callValue, bytes memory methodData) = _getExitCalldata(_strategy, _data, _resultTokensIn, _tokensOut, _minAmountsOut);
    customInfo.strategy.invokeFromIntegration(target, callValue, methodData);

    // Post actions
    (targetAddressP, callValueP, methodDataP) = _getPostActionCallData(_strategy, customInfo.addressParam, _resultTokensIn, 1);
    ...
    emit CustomExited(address(customInfo.strategy), address(customInfo.garden), address(this), customInfo.resultToken);
  }
  ...
  function _validatePreExitCustomData(CustomInfo memory _customInfo) internal view {
    ...
    require(_customInfo.resultTokensInStrategy >= _customInfo.resultTokensInTransaction, "The strategy does not have enough result tokens");
  }
  ...
  function _validatePostExitCustomData(CustomInfo memory _customInfo) internal view {
    require(
      IERC20(_customInfo.resultToken).balanceOf(address(_customInfo.strategy)) == _customInfo.resultTokensInStrategy - _customInfo.resultTokensInTransaction,
